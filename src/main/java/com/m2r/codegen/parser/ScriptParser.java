...
public class ScriptParser extends Parser<DomainList> {
	 ...
	 * DOMAIN			-> CLASS | ENUM
	 * CLASS			-> class <PID> { <ATTRIBUTES_LIST> } | class <PID> {  }
	...
	protected boolean domain() {
		int start = pos;
		return (_class() || reset(start)) ||
				(_enum() || reset(start));
	}

	protected boolean _class() {
		int start = pos;
		return ((term(TokenType.ID, "class") && pid() && term(TokenType.DELIMITER, "{") && attributesList() && term(TokenType.DELIMITER, "}")) || reset(start)) ||
				((term(TokenType.ID, "class") && pid() && term(TokenType.DELIMITER, "{") && term(TokenType.DELIMITER, "}")) || reset(start));
	}
	...
	protected boolean pidlist() {
		int start = pos;
		return ((pid() && term(TokenType.DELIMITER, ";") && pidlist()) || reset(start)) ||
				((pid() && term(TokenType.DELIMITER, ";")) || reset(start));
	}
	...
	protected boolean attributesList() {
		int start = pos;
		return ((atribute() && term(TokenType.DELIMITER, ";") && attributesList()) || reset(start)) ||
				((atribute() && term(TokenType.DELIMITER, ";")) || reset(start));
	}
